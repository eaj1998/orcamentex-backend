...
exports.orderCreate = [
  auth,
  body("customer", "É necessário associar o orçamento à um Cliente")
    .isLength({ min: 1 })
    .trim(),
  body("products", "É necessário adicionar produtos ao orçamento.").custom(
    (value) => {
  ...
  async function (req, res) {
    ...
    if (!order.customer.email)
      return apiResponse.ErrorResponse(
        res,
        new BaseException("Cliente sem e-mail cadastrado!")
      );
...
async function updateHtml(filePath, order) {
  ...
  const productListHTML = await order.products
    .map(
      (productOrder) => `
	<tr>
  ...
  const totalPrice = await order.products.reduce(
